...
static void handle_surface_commit(struct wl_listener *listener, void *data) {
	// We need to rebase cursor after surface_reconfigure, which is called after the commit event.
	// If we rebase cursor earlier, wlr_scene_node_at won't get the session lock scene node.
	cursor_rebase_all();
