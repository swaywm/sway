...
static bool search_render_format(struct search_context *ctx, size_t output_idx) {
	...
	if (cfg->config->render_bit_depth == RENDER_BIT_DEPTH_DEFAULT ||
		cfg->config->render_bit_depth == RENDER_BIT_DEPTH_8) {
		wlr_output_state_set_render_format(state, DRM_FORMAT_XRGB8888);
		return search_mode(ctx, output_idx);
	}

	// We only need to test 10-bit formats from here
	if (render_format_is_10bit(wlr_output->render_format)) {
		// 10-bit was set successfully before, try to save some tests by reusing the format
		wlr_output_state_set_render_format(state, wlr_output->render_format);
		sway_log(SWAY_DEBUG, "Trying with render format %d for: %s", wlr_output->render_format,
			wlr_output->name);
	...
	for (size_t idx = 0; fmts[idx] != DRM_FORMAT_INVALID; idx++) {
		if (fmts[idx] == wlr_output->render_format) {
			// We already tried this one above
			continue;
		}
...
static bool search_adaptive_sync(struct search_context *ctx, size_t output_idx) {
	...
	if (cfg->config->adaptive_sync == 1) {
		sway_log(SWAY_DEBUG, "Trying with adaptive sync enabled for: %s",
	...
	return search_render_format(ctx, output_idx);
}
...
static bool search_valid_config(struct search_context *ctx, size_t output_idx) {
	...
	queue_output_config(cfg->config, cfg->output, state);
	if (!cfg->config->enabled) {
		return search_valid_config(ctx, output_idx+1);
	}

	// Search through our possible configurations, doing a depth-first through
	// adaptive_sync, render_format, modes and the next output's config.
	...
	// We could not get anything to work, try to disable this output to see if
	// we can at least make the outputs before us work.
	sway_log(SWAY_DEBUG, "Trying with disabled output for: %s", backend_state->output->name);
	wlr_output_state_set_enabled(state, false);
	return search_finish(ctx, output_idx);
}
