...
static void arrange_container(struct sway_container *con,
	...
	if (con->view) {
		...
		double content_width = align_to_fractional(scale, con->current.content_width);
		double content_height = align_to_fractional(scale, con->current.content_height);
		...
		width = border_left + content_width + border_right;
		height = border_top + content_height + border_bottom;
...
static void arrange_workspace_tiling(struct sway_workspace *ws,
		double width, double height) {
	float scale = ws->output
		? ws->output->wlr_output->scale
		: 1.0f;
	double gaps = align_to_fractional(scale, ws->gaps_inner);
