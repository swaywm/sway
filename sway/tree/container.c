...
static void set_fullscreen(struct sway_container *con, bool enable) {
	...
	struct wlr_linux_dmabuf_feedback_v1_builder *builder =
		wlr_linux_dmabuf_feedback_v1_builder_create();

	wlr_linux_dmabuf_feedback_v1_builder_set_main_renderer(builder,
		server.renderer);
	wlr_linux_dmabuf_feedback_v1_builder_set_scanout_primary_output(builder,
		output->wlr_output);

	const struct wlr_linux_dmabuf_feedback_v1 *feedback =
		wlr_linux_dmabuf_feedback_v1_builder_get_feedback(builder);
	if (feedback != NULL) {
		wlr_linux_dmabuf_v1_set_surface_feedback(server.linux_dmabuf_v1,
			con->view->surface, feedback);
	}

	wlr_linux_dmabuf_feedback_v1_builder_destroy(builder);
}
