...
static bool keyboard_execute_pointer_keysyms(struct sway_keyboard *keyboard,
		const xkb_keysym_t *pressed_keysyms, size_t keysyms_len,
	...
	for (size_t i = 0; i < keysyms_len; ++i) {
		...
		if (button != 0) {
			dispatch_cursor_button(cursor, NULL, 0, button,
		...
		if (state == WL_KEYBOARD_KEY_STATE_PRESSED && (dx != 0 || dy != 0)) {
			wlr_cursor_move(cursor->cursor, NULL, dx * 10, dy * 10);
...
static void handle_key_event(struct sway_keyboard *keyboard,
	...
	if (!handled) {
		handled = keyboard_execute_pointer_keysyms(keyboard,
