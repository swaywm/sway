...
static void dispatch_watch(int fd, short mask, void *data) {
	wlr_log(L_DEBUG, "Dispatching watch");
...
static dbus_bool_t add_watch(DBusWatch *watch, void *_data) {
	...
	wlr_log(L_DEBUG, "Adding DBus watch fd: %d", fd);
...
static void dispatch_timeout(timer_t timer, void *data) {
	wlr_log(L_DEBUG, "Dispatching DBus timeout");
...
static dbus_bool_t add_timeout(DBusTimeout *timeout, void *_data) {
	...
	if (!timer) {
		wlr_log(L_ERROR, "Cannot allocate memory");
		return FALSE;
	}
	struct sigevent ev = {
	...
	if (timer_create(CLOCK_MONOTONIC, &ev, timer)) {
		wlr_log(L_ERROR, "Could not create DBus timer");
		return FALSE;
	}
	...
	wlr_log(L_DEBUG, "Adding DBus timeout. Interval: %ds %dms",
			interval_sec, interval_msec);
...
static void remove_timeout(DBusTimeout *timeout, void *_data) {
	...
	wlr_log(L_DEBUG, "Removing DBus timeout.");
...
static void dispatch_status(DBusConnection *connection,
		DBusDispatchStatus new_status, void *_data) {
...
static void get_prop_callback(DBusPendingCall *pending, void *_data) {
	...
	if (!reply) {
		wlr_log(L_INFO, "Could not get a property responce");
		goto bail;
	}
	...
	if (dbus_message_get_type(reply) == DBUS_MESSAGE_TYPE_ERROR) {
		...
		wlr_log(L_INFO, "Failure to get property: %s", msg);
	...
	if (dbus_message_iter_get_arg_type(&iter) != DBUS_TYPE_VARIANT) {
		wlr_log(L_ERROR, "Property relpy type incorrect");
	...
	if (!dbus_message_iter_check_signature(&variant, data->sig)) {
		wlr_log(L_INFO, "Property returned has incorrect signatue.");
...
bool dbus_get_prop_async(const char *destination,
	...
	if (!(pending || status)) {
		wlr_log(L_ERROR, "Could not get property (%s) from path "
				"(%s) on iface (%s)",prop, path, iface );
		return false;
	}
...
void dispatch_dbus() {
	...
	if (status != DBUS_DISPATCH_COMPLETE) {
		wlr_log(L_ERROR, "Cannot dispatch dbus events: %d", status);
	}
...
int dbus_init() {
	...
	if (conn == NULL) {
		wlr_log(L_INFO, "Compiled with dbus support, " \
				"but unable to connect to dbus.");
		wlr_log(L_INFO, "swaybar will be unable to display tray icons.");
		return -1;
	}

	dbus_connection_set_exit_on_disconnect(conn, FALSE);
	if (dbus_error_is_set(&error)) {
		wlr_log(L_ERROR, "Cannot get DBus connection: %s\n",
				error.message);
		conn = NULL;
		return -1;
	}

	wlr_log(L_INFO, "Unique name: %s\n", dbus_bus_get_unique_name(conn));
	...
	if (!dbus_connection_set_watch_functions(conn, add_watch, remove_watch,
		...
		wlr_log(L_ERROR, "Failed to activate DBUS watch functions");
		return -1;
	}

	if (!dbus_connection_set_timeout_functions(conn, add_timeout,
				remove_timeout, NULL, NULL, NULL)) {
		dbus_connection_set_watch_functions(conn,
				NULL, NULL, NULL, NULL, NULL);
		dbus_connection_set_timeout_functions(conn,
				NULL, NULL, NULL, NULL, NULL);
		wlr_log(L_ERROR, "Failed to activate DBUS timeout functions");
		return -1;
	}
