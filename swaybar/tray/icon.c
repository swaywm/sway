...
static char *find_theme_dir(const char *theme) {
	...
	if ((basedir = getenv("XDG_DATA_DIRS"))) {
		if (!(basedir = strdup(basedir))) {
			wlr_log_errno(L_ERROR, "Path too long to render");
			goto fail;
		}
...
static list_t* find_theme_subdirs(const char *theme_dir) {
	...
	FILE *index = NULL;
	if (!path) {
		wlr_log(L_ERROR, "Failed to allocate memory");
		goto fail;
	}
	...
	index = fopen(path, "r");
	if (!index) {
		wlr_log(L_ERROR, "Could not open file: %s", path);
		goto fail;
	}
...
cairo_surface_t *find_icon(const char *name, int size) {
	...
	if (!pixbuf) {
		wlr_log(L_ERROR, "Failed to load icon image: %s", err->message);
	}
	...
	if (!image) {
		wlr_log(L_ERROR, "Could not read icon image");
		return NULL;
	}
