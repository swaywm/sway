 ...
 *
 * FIXME Treat object path items like KDE does
 */
...
static struct ObjPathItem *create_obj_path_item(const char *unique_name,
		const char *obj_path) {
...
static int obj_path_unique_name_cmp(const void *_item,
		const void *_unique_name) {
...
static void obj_path_item_registered_signal(DBusConnection *connection,
		const struct ObjPathItem *item) {
	DBusMessage *signal = dbus_message_new_signal(
			"/StatusNotifierWatcher",
...
static void register_item(DBusConnection *connection, DBusMessage *message) {
	...
	if (!dbus_message_get_args(message, &error,
				DBUS_TYPE_STRING, &name,
				DBUS_TYPE_INVALID)) {
		wlr_log(L_ERROR, "Error parsing method args: %s", error.message);
	}

	wlr_log(L_INFO, "RegisterStatusNotifierItem called with \"%s\"", name);
	...
	if (!dbus_validate_bus_name(name, NULL)) {
		if (dbus_validate_path(name, NULL)) {
			...
			wlr_log(L_INFO, "This item is not valid, we cannot keep track of it.");
			return;
		}
	} else {
...
static void register_host(DBusConnection *connection, DBusMessage *message) {
	...
	wlr_log(L_INFO, "RegisterStatusNotifierHost called with \"%s\"", name);
...
static void get_property(DBusConnection *connection, DBusMessage *message) {
	...
	if (!dbus_message_get_args(message, &error,
				...
				DBUS_TYPE_INVALID)) {
		wlr_log(L_ERROR, "Error parsing prop args: %s", error.message);
		return;
	}

	if (strcmp(property, "RegisteredStatusNotifierItems") == 0) {
		wlr_log(L_INFO, "Replying with items");
		DBusMessage *reply;
		reply = dbus_message_new_method_return(message);
		...
		wlr_log(L_INFO, "Replying with ObjPathItems");
		DBusMessage *reply;
		reply = dbus_message_new_method_return(message);
		DBusMessageIter iter;
...
static DBusHandlerResult signal_handler(DBusConnection *connection,
	...
	if (dbus_message_is_signal(message, "org.freedesktop.DBus", "NameOwnerChanged")) {
		...
		if (!dbus_message_get_args(message, NULL,
				...
				DBUS_TYPE_INVALID)) {
			wlr_log(L_ERROR, "Error getting LostName args");
			return DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
		}
		...
		if ((index = list_seq_find(items, (int (*)(const void *, const void *))strcmp, name)) != -1) {
			wlr_log(L_INFO, "Status Notifier Item lost %s", name);
		...
		if ((index = list_seq_find(hosts, (int (*)(const void *, const void *))strcmp, name)) != -1) {
			wlr_log(L_INFO, "Status Notifier Host lost %s", name);
		...
		while ((index = list_seq_find(object_path_items, obj_path_unique_name_cmp, name)) != -1) {
			...
			wlr_log(L_INFO, "ObjPathItem lost %s", item->obj_path);
...
int init_sni_watcher() {
	...
	dbus_error_init(&error);
	if (!conn) {
		wlr_log(L_ERROR, "Connection is null, cannot initiate StatusNotifierWatcher");
		return -1;
	}
	...
	if (status == DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER) {
		wlr_log(L_DEBUG, "Got watcher name");
	} else if (status == DBUS_REQUEST_NAME_REPLY_IN_QUEUE) {
		wlr_log(L_INFO, "Could not get watcher name, it may start later");
	}
	if (dbus_error_is_set(&error)) {
		wlr_log(L_ERROR, "dbus err getting watcher name: %s", error.message);
		return -1;
	}
	...
	status = dbus_bus_request_name(conn, "org.kde.StatusNotifierWatcher",
			...
			&error);
	if (status == DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER) {
		wlr_log(L_DEBUG, "Got kde watcher name");
	} else if (status == DBUS_REQUEST_NAME_REPLY_IN_QUEUE) {
		wlr_log(L_INFO, "Could not get kde watcher name, it may start later");
	}
	if (dbus_error_is_set(&error)) {
		wlr_log(L_ERROR, "dbus err getting kde watcher name: %s", error.message);
		return -1;
	}
	...
	if (dbus_error_is_set(&error)) {
		wlr_log(L_ERROR, "DBus error getting match args: %s", error.message);
	}
